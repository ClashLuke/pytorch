...
def bmm(
    ...
    if self.device.type == "cpu":
        if (
            guard_size_oblivious(self.size(0) == 1)
            and guard_size_oblivious(batch2.size(0) == 1)
            and config.is_fbcode()
        ):
...
def addmm(
    ...
    if self.device.type == "cpu":
        ...
        if (
            ...
            and (
                definitely_true(mat2.size(0) <= 16)
                or (definitely_true(mat2.size(0) <= 64) and config.is_fbcode())
            )
...
def mm(
    ...
    if self.device.type == "cpu":
        ...
        if (
            guard_size_oblivious(self.size(0) <= 16)
            and guard_size_oblivious(input2.size(-1) == 1) 
            and config.is_fbcode()
        ):
            counters["inductor"]["decompose_mm"] += 1
        ...
        if (
            guard_size_oblivious(self.size(0) <= 16)
            and guard_size_oblivious(input2.size(-1) == 2)
            and config.is_fbcode()
        ):
            counters["inductor"]["decompose_mm"] += 1
