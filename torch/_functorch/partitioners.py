...
def solve_min_cut(
    ...
    def should_ban_recomputation(node):
        ...
        if min_cut_options.ban_if_materialized_backward and is_materialized_backwards(
            ...
            logger.debug("materialized backwards: %s %s", node, tuple(node.users))
            return True
    ...
    try:
        ...
        logger.info("Failed to compute min-cut on following graph:")
...
def visualize_min_cut_graph(nx_graph):
    ...
    for edge in dot_graph.get_edges():
        ...
        if weight == float("inf"):
            edge.set_color("red")
    logger.info("Visualizing the failed graph to min_cut_failed.svg")
...
def choose_saved_values_set(
    ...
    if config.visualize_memory_budget_pareto:
        ...
        logger.warning("Generated Pareto frontier curve at %s", fig_name)
...
def min_cut_rematerialization_partition(
    ...
    *,
    ...
    if AOT_PARTITIONER_DEBUG:
        ...
        logger.debug(
            "Theoretical Activations Stored: %.2f GB", total_activations_size_gb
        )

        # Log theoretical per activation storage sizes
        logger.debug("Theoretical Per Activation Storage Sizes: %s", sorted_sizes)
        fw_module_nodes = {
        ...
        logger.debug(
            "# remat/fw/bw: %d/%d/%d",
            len(remat_nodes), len(fw_module_nodes), len(bw_module_nodes)
        )   
        rematerialized_ops = sorted(counts.items(), key=lambda x: x[1], reverse=True)
        logger.debug("Count of Ops Rematerialized: %s", rematerialized_ops)
    return fw_module, bw_module
...
def draw_graph(
    ...
    logger.info("Writing FX graph to file: %s%s", base, ext)    
